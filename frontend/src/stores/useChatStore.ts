    ...
    (set, get) => ({
      ...
      addMessage: async (message) => {
        try {
          ...
          let prevItems = get().messages[convoId]?.items ?? []; // Lấy danh sách tin nhắn hiện tại của cuộc trò chuyện
      ...
      markAsSeen: async () => {
        try {
          ...
          // Nếu số tin nhắn chưa đọc của user bằng 0 thì return
            ...
            conversations: state.conversations.map((c) =>
              c._id === activeConversationId && c.lastMessage
                ? {
                    ...
                    unreadCounts: {
                      ...c.unreadCounts,
                      [user._id]: 0, // Đặt số tin nhắn chưa đọc của user thành 0
                    },
                  }
                : c,
            ),
